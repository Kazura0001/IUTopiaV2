...
inline void checkTimeBan(ENetPeer* peer, string username)
{
	...
	if (exist)
	{
		...
		if (fd.fail())
		{
			...
			Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + username + "' is time banned. `5Please send this screenshot to IUTopia developers.");
	...
	if (exist)
	{
		...
		if (experimental::filesystem::exists("autoban/ip/" + ip + ".txt"))
		{
			...
			if (ifs.fail())
			{
				...
				Player::OnConsoleMessage(peer, "`5[Stream-ERROR]: `@ failed to check whether '" + static_cast<PlayerInfo*>(peer->data)->rawName + "' is auto banned by system. `5Please send this screenshot to IUTopia developers.");
				if (!static_cast<PlayerInfo*>(peer->data)->Console) enet_peer_disconnect_later(peer, 0);
			}
			else
			{
...
void ApplyLockPacket(ENetPeer* _peer, int x, int y, int id, int locksize, int lockowner)
{
	...
	enet_peer_send(_peer, 0, packetenet);
