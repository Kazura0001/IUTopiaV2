...
void sendStatex(ENetPeer* peer, PlayerInfo* info) {
	//return; // TODO
		...
		++currentPeer)
	{
		...
		if (isHere(peer, currentPeer)) {
			...
			int var = 0x808040; // placing and breking 0x808040 
			memcpy(raw + 1, &var, 3);
	...
	case 18: /*punch*/
		{
			...
			if (tile == 202)
			{
				...
				sendTileData(peer, x, y, 0x10, world->items[squaresign].foreground, world->items[squaresign].background, lockTileDatas(0x20, ((PlayerInfo*)(peer->data))->userID, 0, 0, false, 100));
			}
			if (isSmallLock) {
			...
			if (world->items[x + (y * world->width)].foreground == 1790)
			{
				//if (static_cast<PlayerInfo*>(peer->data)->rawName == world->owner || isDev(peer))
				{
					...
					else if (cQuest == "Knight")
					{
						...
						{
							auto s1Quest = 0;
							ifstream basic_ifstream("quests/knight/step1/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							basic_ifstream >> s1Quest;
							basic_ifstream.close();
							std::ifstream ifsz("gemdb/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							std::string content((std::istreambuf_iterator<char>(ifsz)), (std::istreambuf_iterator<char>()));
							...
							Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_label_with_icon|big|`#Quest For The Knight``|left|7734|0|0|\n\nadd_spacer|small|\nadd_textbox|`5Your current step: `^" + to_string(sQuest) + "/5|\nadd_textbox|`5Step 1: Welcome adventurer, your first `^Quest `5Will be to bring me `43m `9Gems`5, Hope that not too much for you`5!|\nadd_textbox|`5Progress: `^" + to_string(s1Quest) + "/3000000 `5Gems|\nadd_button|s1sgemsgive|`9Give `^" + to_string(cangive) + " `9Gems|\nadd_button|chc0|`9Close|0|0|\nadd_quick_exit|");
						}
						else if (sQuest == 2)
						{
							auto s2Quest = 0;
							ifstream basic_ifstream("quests/knight/step2/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							basic_ifstream >> s2Quest;
							basic_ifstream.close();
							if (s2Quest < 50000)
							{
						...
						{
							auto s3Quest = 0;
							ifstream basic_ifstream("quests/knight/step3/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							basic_ifstream >> s3Quest;
							basic_ifstream.close();
							SearchInventoryItem(peer, 1486, 5, iscontains);
							if (!iscontains)
							...
							{
								Player::OnDialogRequest(peer, "set_default_color|`o\n\nadd_label_with_icon|big|`#Quest For The Katana``|left|7734|0|0|\n\nadd_spacer|small|\nadd_textbox|`5Your current step: `^" + to_string(sQuest) + "/5|\nadd_textbox|`5Step 3: I challenge you to bring me `45 `5Growtokens`5!|\nadd_textbox|`5Progress: `^5/5 `5Growtoken|\nadd_button|s4scomplete|`9Complete!|\nadd_button|chc0|`9Close|0|0|\nadd_quick_exit|");
							}
						}
						else if (sQuest == 4)
						{
							auto s4Quest = 0;
							ifstream basic_ifstream("quests/knight/step4/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							basic_ifstream >> s4Quest;
							basic_ifstream.close();
							auto iscontains = false;
							SearchInventoryItem(peer, 2592, 1, iscontains);
						...
						{
							auto s5Quest = 0;
							ifstream basic_ifstream("quests/knight/step5/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
							basic_ifstream >> s5Quest;
							basic_ifstream.close();
							auto iscontains = false;
							SearchInventoryItem(peer, 1794, 1, iscontains);
		...
		if (static_cast<PlayerInfo*>(peer->data)->PunchPotion && tile == 18 || static_cast<PlayerInfo*>(peer->data)->cloth_necklace == 6260 && tile == 18 && static_cast<PlayerInfo*>(peer->data)->Subscriber)
		{
			if (world->items[x + (y * world->width)].foreground == 4762 || world->items[x + (y * world->width)].foreground == 7382 || world->items[x + (y * world->width)].foreground == 9468 || world->items[x + (y * world->width)].foreground == 10024 || world->items[x + (y * world->width)].foreground == 10028 || world->items[x + (y * world->width)].foreground == 9460 || world->items[x + (y * world->width)].foreground == 340 || world->items[x + (y * world->width)].foreground == 954 || world->items[x + (y * world->width)].foreground == 5666)
			{
				if (isWorldOwner(peer, world) || isDev(peer) || world->isPublic || world->owner == "")
				{
					...
					{
						SendFarmableDrop(peer, 15000);
						SendGuildPoints(peer, 90);
						SendXP(peer, 10);
						SendDropSeed(peer, x, y, world->items[x + (y * world->width)].foreground);
					}
					...
					{
						string dialog = "add_label_with_icon|big|`wSalesman Shops|left|2398|\nadd_textbox|`oEyyoo `9Growtopians`o!! You can buy many `2RARE `witem from me for cheaps price!!|left|\nadd_textbox|`oWhich you want buy!!|left|\nadd_button|zeus1|`oZeus' Lightning Bolt``|noflags|0|0|\nadd_button|autumn1|`oAutumn Wings``|\nadd_button|angel1|`oGolden Angel Wings``|\nadd_button|mercy1|`oAngel of Mercy's Wings``|\nadd_textbox|`2Packs items change`o|\nadd_button|packitem|`oPacks Items Changer``|\nadd_spacer|small|\nend_dialog|scan2|`wCancel|`wOK|";
					...
					{
						SendFarmableDrop(peer, 3000);
						SendGuildPoints(peer, 100);
						SendXP(peer, 180);
						SendDropSeed(peer, x, y, world->items[x + (y * world->width)].foreground);
					}
					if (world->items[x + (y * world->width)].foreground != 0)
					...
					if (static_cast<PlayerInfo*>(peer->data)->haveGrowId) {
						if (world->items[x + (y * world->width)].foreground == 2398) {
							if (static_cast<PlayerInfo*>(peer->data)->level <= 1) {
								Player::OnTextOverlay(peer, "You need 2 levels to access hourly reward!");
								return;
							}
							//hourly
							if (!std::experimental::filesystem::exists("hourly/" + PlayerDB::getProperName(static_cast<PlayerInfo*>(peer->data)->rawName) + ".txt")) {
								ofstream savebantime("hourly/" + static_cast<PlayerInfo*>(peer->data)->rawName + ".txt");
								savebantime.close();
							}
							ifstream fd("hourly/" + PlayerDB::getProperName(static_cast<PlayerInfo*>(peer->data)->rawName) + ".txt");
							...
							string bobo = "\nadd_smalltext|`5(You claimed already, wait `w" + OutputBanTime(calcBanDuration(timeremaining)) + " `5to claim again)|left|";
							auto bantimeleft = calcBanDuration(timeremaining);
							if (bantimeleft < 1) {
								bobo = "\nadd_button|claimhour|`wClaim `225,000 `wgems|left|";
							}
							Player::OnDialogRequest(peer, "\nadd_label_with_icon|big|`$Hourly Reward|left|2398|\nadd_spacer|small|\nadd_label|small|`9Information : `$Claiming your reward every hour will gave u 25,000 gems for free! Once you claimed it there will be 1 hour cooldown or timeout. You may claim it again after 1 hour!|left|\nadd_spacer|small|" + bobo + "\nadd_spacer|small|\nadd_quick_exit|\nend_dialog||Thanks|");
						}
					}
					else {
						Player::OnTextOverlay(peer, "Create your GrowID first!");
					}
				}
