...
inline void saveAllWorlds() // 
{
	...
	Sleep(200);
	GamePacket p = packetEnd(appendString(appendString(createPacket(), "OnConsoleMessage"), "`9If you have any question, please do `4/report. `5Moderator will help you as fast as possible."));
	...
	if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
	{
		if (static_cast<PlayerInfo*>(peer->data)->rawName == getPlyersWorld(peer)->owner)
		{
			...
			if (static_cast<PlayerInfo*>(peer->data)->adminLevel == 5)
			{
				if (static_cast<PlayerInfo*>(peer->data)->NickPrefix != "")
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`2[ADMIN]@" + static_cast<PlayerInfo*>(peer->data)->NickPrefix + ". `2" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
				else
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`2" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
		...
		else
		{
			if (static_cast<PlayerInfo*>(peer->data)->adminLevel == 5)
			{
				...
				if (static_cast<PlayerInfo*>(peer->data)->NickPrefix != "")
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`2[ADMIN]@" + static_cast<PlayerInfo*>(peer->data)->NickPrefix + ". " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
				else
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`2[ADMIN]@" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
			}
		}
	}
	...
	if (static_cast<PlayerInfo*>(peer->data)->haveGrowId)
	{
		if (static_cast<PlayerInfo*>(peer->data)->rawName == getPlyersWorld(peer)->owner)
		{
			if (static_cast<PlayerInfo*>(peer->data)->adminLevel == 6)
			{
				...
				if (static_cast<PlayerInfo*>(peer->data)->NickPrefix != "")
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`b[Dev]" + static_cast<PlayerInfo*>(peer->data)->NickPrefix + ". `2" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
				else
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`2" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
		...
		else
		{
			if (static_cast<PlayerInfo*>(peer->data)->adminLevel == 6)
			{
				...
				if (static_cast<PlayerInfo*>(peer->data)->NickPrefix != "")
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`b[Dev]" + static_cast<PlayerInfo*>(peer->data)->NickPrefix + ". " + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
				else
				{
					static_cast<PlayerInfo*>(peer->data)->displayName = "`b[Dev]" + static_cast<PlayerInfo*>(peer->data)->tankIDName;
				}
			}
		}
	}
...
inline void DailyRewardCheck(ENetPeer* peer)
{
	...
	if (static_cast<PlayerInfo*>(peer->data)->startkit == false)
	{
		...
		fs.close();
		bool success = false;
		SaveItemMoreTimes(7382, 200, peer, success);
		SaveItemMoreTimes(1796, 50, peer, success);
		Player::onGiveGems(peer, 50000);
		static_cast<PlayerInfo*>(peer->data)->startkit = true;
		Player::OnConsoleMessage(peer, "`^You received 200 Onyx Block and 50 Diamond Lock & 50000 Gems for you to START");
	}
}
